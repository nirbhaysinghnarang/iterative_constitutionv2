...
export async function getData(): Promise<Dataset> {
  ...
  try {
      ...
      const scenarios: scenario[] = parsedData.slice(0,3).map((row: any) => ({
      ...
      categories.forEach((scens, category) => {
          ...
          const numTrain = Math.max(minPerCategory, Math.floor(trainSize * (scens.length / scenarios.length)));
          const numTest = Math.max(minPerCategory, Math.floor(testSize * (scens.length / scenarios.length)));

          train = train.concat(scens.slice(0, numTrain));
          test = test.concat(scens.slice(numTrain, numTrain + numTest));

          // Capture the indices
          trainIndices = trainIndices.concat(scens.slice(0, numTrain).map(s => scenarios.indexOf(s)));
          testIndices = testIndices.concat(scens.slice(numTrain, numTrain + numTest).map(s => scenarios.indexOf(s)));
      });
